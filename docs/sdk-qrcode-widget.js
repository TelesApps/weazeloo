(function() {
  ...
  // 3. Utility to load an external script and return a Promise
  ...
  // 4. Main function: fetch transaction, inject snippet, generate QR
  function generateQrCode() {
    ...
    fetch(endpointUrl, {
      ...
      .then(res => {
        // (A) Inject the HTML snippet
        const htmlSnippet = res.html;
        const fragment    = document.createRange().createContextualFragment(htmlSnippet);
        const containerEl = document.getElementById(containerId);
        containerEl.appendChild(fragment);

        // (B) If instnt is available, wire up the user’s event callback and get the invitation URL
        if (window.instnt) {
          // --- 1) If user provided a global callback name, wire it up
          ...
          // --- 2) Get the invitation URL and generate the QR code
  ...
  // 5. Load jQuery, then qr-code-styling, then run generateQrCode()
